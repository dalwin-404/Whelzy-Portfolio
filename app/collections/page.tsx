...
export default function CollectionsPage() {
  ...
  const [selectedImage, setSelectedImage] = useState<typeof allCollectionsData[0] | null>(null);
          ...
          <motion.div layout className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 md:gap-6 space-y-4 md:space-y-6">
            <AnimatePresence mode="popLayout">
              {filteredItems.map((item) => (
                <motion.article
                  key={item.id}
                  layoutId={`collections-image-${item.id}`}
                  data-cursor="EXPAND"
      ...
      <AnimatePresence>
        {selectedImage && (
          ...
          >
            ...
            <motion.div
              layoutId={`collections-image-${selectedImage.id}`}
